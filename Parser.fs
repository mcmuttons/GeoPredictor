...
module Parser =
    ...
    let toMaterialOut useSymbols material =
        match material with
        | Carbon -> if useSymbols then "C" else "Carbon"
        | Vanadium -> if useSymbols then "V" else "Vanadium"
        | Niobium -> if useSymbols then "Nb" else "Niobium"
        | Yttrium -> if useSymbols then "Y" else "Yttrium"
        | Phosphorus -> if useSymbols then "P" else "Phosphorus"
        | Chromium -> if useSymbols then "Cr" else "Chromium"
        | Molybdenum -> if useSymbols then "Mo" else "Molybdenum"
        | Technetium -> if useSymbols then "Tc" else "Technetium"
        | Sulphur -> if useSymbols then "S" else "Sulphur"
        | Manganese -> if useSymbols then "Mn" else "Manganese"
        | Cadmium -> if useSymbols then "Cd" else "Cadmium"
        | Ruthenium -> if useSymbols then "Ru" else "Ruthenium"
        | Iron -> if useSymbols then "Fe" else "Iron"
        | Zinc -> if useSymbols then "Zn" else "Zinc"
        | Tin -> if useSymbols then "Sn" else "Tin"
        | Selenium -> if useSymbols then "Se" else "Selenium"
        | Nickel -> if useSymbols then "Ni" else "Nickel"
        | Germanium -> if useSymbols then "Ge" else "Germanium"
        | Tungsten -> if useSymbols then "W" else "Tungsten"
        | Tellurium -> if useSymbols then "Te" else "Tellurium"
        | Rhenium -> if useSymbols then "Re" else "Rhenium"
        | Arsenic -> if useSymbols then "As" else "Arsenic"
        | Mercury -> if useSymbols then "Hg" else "Mercury"
        | Polonium -> if useSymbols then "Po" else "Polonium"
        | Lead -> if useSymbols then "Pb" else "Lead"
        | Zirconium -> if useSymbols then "Zr" else "Zirconium"
        | Boron -> if useSymbols then "B" else "Boron"
        | Antimony -> if useSymbols then "Sb" else "Antimony"
